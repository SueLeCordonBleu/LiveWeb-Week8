...
io.sockets.on('connection',
	...
	function (socket) {
		...
		// 	var strippedImage = theData.slice(theData.indexOf(searchFor) + searchFor.length); //把header 去掉。
		//
		// 	//需要把base64 换成 binary.Buffer 可以接受base64, convert it to binary.
		// 	var binaryImage = new Buffer(strippedImage, 'base64');
		// 	fs.writeFileSync(__dirname + '/theimage' + Date.now() + '.jpg', binaryImage); //Sync 意味着不需要callback
		// });

		socket.on('video', function(theData) {
			fs.writeFileSync(__dirname + '/thevideo' + Date.now() + '.webm', theData);
		});
